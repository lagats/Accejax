...
/*
	ACCEJAX OVERVIEW
	example of calling accejax:
                accejax.init({
                  item: [
                    {
                      container: "#maincontainer", // container can be an ID or a Class
                      target: ".ajax-item__link:internal",
  ...
  item: 				(Required) list of items to run accejax for, structure should be an array of objects:
                  [{
                      container: "...", 	// Container of area to load from new page into current 
                      												page (needs matching ID)
                      target: "...", 			// The item to add the click event to
                      targetwrap: "..." 	// (optional) Move the 'hovering' effect on page return to this 
                      												item - defailt adds to the target
                    },
                    {...}
                  ]
  skipcontent: 	(Required) Points focus back to this menu when ajax load (as if it was a new page), 
  								& adds an item here to jump to where you left off.
  classes:			Customise the classes that get added to the DOM
*/
...
var accejax = {
    ...
    listeners: function(){
        ...
        for (var i = 0; i < len; i++){
          ...
          (async function () { 
            var key = i;
            $(document).on("click", existingItems[i]["target"], function(e) { 
              // prevent click event
              e.preventDefault();
    ...
    bookmark: function(clickTarget, targetWrap){
        ...
        var bookmarkClassName = this.store["classes"]["bookmark_name"];
      	if (clickTarget) {
          $(bookmarkClass).removeClass(bookmarkClassName);
          $(bookmarkClass+"_link").removeClass(bookmarkClassName+"_link");
        }
        if (clickTarget && targetWrap) {
          ...
          $(bookmarkClass).addClass(hoverClassName); 
          setTimeout(function(){ $(bookmarkClass).removeClass(hoverClassName); },400);
        }
    },
...
window.addEventListener("popstate", function(e) {
  try {
    ...
    return console.error("Accejax Err - History state is undefined");
  }
  var state = e.state;
...
/* Checking for internal or external links */
$.expr[":"].external = function(a) {
  ...
  return (
    ...
    href.search(window.location.hostname) == -1
  );
};
